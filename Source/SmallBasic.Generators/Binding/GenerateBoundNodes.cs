// <copyright file="GenerateBoundNodes.cs" company="MIT License">
    ...
    public sealed class GenerateBoundNodes : BaseConverterTask<BoundNodeCollection>
    {
        ...
        private void ValidateNode(BoundNodeCollection model, BoundNode node)
        {
            ...
            foreach (var member in node.Members)
            {
                ...
                {
                    this.LogError($"Member '{node.Name}.{member.Name}' must end with 'Opt' if it was optional, and vice versa.");
                }
            ...
            if (node.IsAbstract)
            {
                ...
                {
                    this.LogError($"Abstract node '{node.Name}' name should start with 'BaseBound'.");
                }

                if (!node.Syntax.IsDefault())
                {
                    this.LogError($"Abstract node '{node.Name}' should not define 'Syntax'.");
                }
            }
            else
            {
                ...
                {
                    this.LogError($"Node '{node.Name}' should start with 'Bound'.");
                }

                if (string.IsNullOrWhiteSpace(node.Syntax))
                {
                    this.LogError($"Node '{node.Name}' should define 'Syntax'.");
                }
            ...
            {
                this.LogError($"Node '{node.Name}' name should not contain 'Syntax'.");
            }
        }
        ...
        private IEnumerable<Member> GetParentMembers(BoundNodeCollection model, string parentName)
        {
            ...
            if (parent.IsDefault())
            {
                this.LogError($"Cannot find parent node '{parentName}'.");
                yield break;
            }
